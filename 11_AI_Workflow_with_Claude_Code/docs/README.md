...
- **`claude/`** — Claude Code teaching artifacts: **[`claude/prompts/README.md`](claude/prompts/README.md)** (**human developer setup before any prompts**, directory listing, and what each **`VIDEO_*`** module covers), **numbered `VIDEO_*` prompts** under **`claude/prompts/`** (hands-on build order), **[`claude/GLOBAL.CLAUDE.md`](claude/GLOBAL.CLAUDE.md)** (reference guidance), and **[`claude/README.md`](claude/README.md)** (**Claude Code features used in this chapter** + prompt index and order).
...
| **[`claude/prompts/README.md`](claude/prompts/README.md)** | **Human developer setup** (GitHub, PAT, `.env`, `gh`, baseline push, required reading) **before** opening any **`VIDEO_*`** file; directory listing; short description of each prompt module. |
| **[`claude/prompts/`](claude/prompts/)** (folder) | **`VIDEO_*_PROMPTS.md`** — prompts to paste into Claude Code in **lexicographic** order; start at **`VIDEO_1_PROMPTS.md`** after completing **[`prompts/README.md`](claude/prompts/README.md)**. |
...
5. **[`claude/prompts/README.md`](claude/prompts/README.md)** — complete **Human developer setup** (machine, GitHub repo, secrets, **`gh`**, baseline **`git push`**) and read the **module summaries** so you know what each **`VIDEO_*`** file covers.
6. **`claude/prompts/`** — run **`VIDEO_1`** through **`VIDEO_4.2`** in **lexicographic order** (same order as listed in `claude/README.md` and **`prompts/README.md`**).
7. **Harness teaching artifacts** — **[`claude/build-notes/`](claude/build-notes/)** (per-module **`VIDEO_*.md`** after you run the prompts) and **[`claude/CLAUDE_CODE_BUILD_LOG.md`](claude/CLAUDE_CODE_BUILD_LOG.md)** (cumulative); the **`log-claude-build`** procedure is defined in **[`claude/templates/log-claude-build.skill.md`](claude/templates/log-claude-build.skill.md)** and installed into **`.claude/skills/`** during **VIDEO_1** (see **[`build-notes/README.md`](claude/build-notes/README.md)**).

The prompts assume you have already internalized the **master `reqs/`** reading (especially before **`VIDEO_1`**, which tells Claude to read those specs first) and completed **human developer setup** in **[`claude/prompts/README.md`](claude/prompts/README.md)** (Git, GitHub remote, **`.env`**, **`gh`**, baseline push).
...
| 1.7 | **`claude/prompts/README.md`**: human developer setup before prompts; layout table row + hands-on reading order step **5**; cross-links from chapter **`README.md`** and **`docs/claude/README.md`**. |
